...
class DBAccess
  ...
  #add the web_urls for which reports need to to be generated
   def persistURLS(url,rName,pg_urls,rEmail)
      ...
      time       = Time.now
      submit     = Submit.create(web_url: url,report_name: rName,pg_urls: page_value,submit_date: time.strftime("%Y-%m-%d"),email_id: rEmail,report_run_status: 'submit')
   end 
  ...
  #get all requested reports
    def getReportRequests()
      rep_req = Submit.where(report_run_status: ['submit','running'])
      request = Array.new
      rep_req.each do |req|
        rep_req = {
          "id"          => req.id,
